...
def bench_single_threaded_medium() capturing:
    try:
...
def bench_multi_threaded_medium() capturing:
    try:
        var in_csv: Path = cwd().joinpath("tests/datablist/people-100000.csv")
...
def bench_single_threaded_small() capturing:
    try:
...
def bench_multi_threaded_small() capturing:
    try:
        var in_csv: Path = cwd().joinpath("tests/datablist/organizations-1000.csv")
        var _ = CsvReader(in_csv)
    except:
...
def bench_single_threaded_large() capturing:
    try:
...
def bench_multi_threaded_large() capturing:
    try:
        var in_csv: Path = cwd().joinpath("tests/datablist/products-2000000.csv")
        var _ = CsvReader(in_csv)
    except:
...
def main():
    ...
    for _ in range(10):
        ...
        time_multi_medium += Float64(elapsed) / 1000000
    ...
    for _ in range(5):  # Fewer iterations for large file
        ...
        time_multi_large += Float64(elapsed) / 1000000
